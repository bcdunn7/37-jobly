...
describe("POST /users", function () {
  ...
  test("anauth for non admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("POST /users/:username/jobs/:id", function () {
  ...
  test("works for curr user", async function () {
    const resp = await request(app)
      .post(`/users/u1/jobs/${testJobIds[0]}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(201);
  ...
  test("unauth for not curr user or admin", async function () {
    const resp = await request(app)
  ...
  test("not found if no user", async function () {
    const resp = await request(app)
      .post(`/users/notAUser/jobs/${testJobIds[0]}`)
      .set("authorization", `Bearer ${uAToken}`);
...
describe("GET /users/:username", function () {
  ...
  test("unauth for not curr user nor admin", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  test("works for curr users", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("works for curr user: set new password", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
